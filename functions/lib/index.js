...
exports.resendOrderEmail = exports.onOrderStatusUpdate = void 0;
...
const RESEND_API_KEY = ((_b = functions.config().resend) === null || _b === void 0 ? void 0 : _b.key) || '';
const db = admin.firestore();
const resend = new resend_1.Resend(RESEND_API_KEY);
sgMail.setApiKey(SENDGRID_API_KEY);
// Email templates for different platforms
const getEmailTemplate = (order, deliveryValue, platform, deliveryType) => {
    const isPSN = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('playstation')) || order.product.toLowerCase().includes('playstation');
    const isSteam = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('steam')) || order.product.toLowerCase().includes('steam');
    const isNitro = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('discord')) || order.product.toLowerCase().includes('discord');
    const isSpotify = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('spotify')) || order.product.toLowerCase().includes('spotify');
    const isRoblox = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('roblox')) || order.product.toLowerCase().includes('roblox');
    const isBrawlStars = (platform === null || platform === void 0 ? void 0 : platform.toLowerCase().includes('brawl')) || order.product.toLowerCase().includes('brawl');
    // Email subject
...
const sendOrderEmail = async (email, templateData) => {
    try {
        // Try SendGrid first if configured
        if (SENDGRID_API_KEY) {
            const msg = {
                to: email,
                from: 'orders@theturkishshop.com',
        ...
        // Fall back to Resend if configured
            ...
            try {
                const emailResponse = await resend.emails.send({
                    from: 'The Turkish Shop <orders@theturkishshop.com>',
                    to: [email],
                    subject: templateData.subject,
                    html: templateData.html,
                    text: templateData.text,
...
exports.onOrderStatusUpdate = functions.firestore
    ...
    if (beforeData.status !== 'delivered' &&
        afterData.status === 'delivered' &&
        afterData.deliveryValue) {
        try {
            // Get email template based on platform and delivery type
            const templateData = getEmailTemplate(afterData, afterData.deliveryValue, afterData.platform, afterData.deliveryType);
            // Send email
...
exports.resendOrderEmail = functions.https.onCall(async (data, context) => {
    // Check if user is admin
    if (!context.auth || !(context.auth.token.admin === true)) {
        throw new functions.https.HttpsError('permission-denied', 'Only admin users can resend order emails');
    }
    const { orderId } = data;
    if (!orderId) {
        throw new functions.https.HttpsError('invalid-argument', 'The function must be called with an orderId argument');
    }
    try {
        // Get order data
        ...
        if (!orderData.deliveryValue) {
            throw new functions.https.HttpsError('failed-precondition', 'Order does not have delivery value set');
        }
        // Get email template
        const templateData = getEmailTemplate(orderData, orderData.deliveryValue, orderData.platform, orderData.deliveryType);
        // Send email
        ...
        throw new functions.https.HttpsError('internal', error instanceof Error ? error.message : 'Unknown error resending email');
    }
});
